...
def _convert_triple(
    ...
    qg = QueryGraph.model_validate(qgraph)
    edge = next(iter(qg.edges.values()), None)
    if edge is None:
        raise ValueError("Query graph must contain exactly one edge.")
    in_node = qg.nodes[edge.subject]
    out_node = qg.nodes[edge.object]
...
def _convert_batch_triple(
    es_transpiler: ElasticsearchTranspiler, qgraphs: list[QueryGraphDict]
) -> list[ESPayload]:
...
def check_list_fields(reference: list[str], against: list[str]):
    for ref, ag in zip(reference, against, strict=False):
...
def remove_biolink_prefixes(input: list[str]):
...
def verify_expanded_multi_qualifiers(
    ...
    for entry in must_entries:
        types_in_entry = {k for term in entry["bool"]["should"] for k in term["term"]}
...
def check_single_query_payload(q_graph: QueryGraphDict, generated_payload: ESPayload):
    ...
    sides: list[SideType] = ["subject", "object"]
    ...
    def add_to_required_fields(_field: str, _side: SideType):
...
async def test_convert_results(es_transpiler: ElasticsearchTranspiler):
    qgraph = QueryGraph.model_validate(BASE_QGRAPH)
    result = es_transpiler.convert_results(qgraph, SIMPLE_ES_HITS)
...
async def test_convert_batch_results(es_transpiler: ElasticsearchTranspiler):
    qgraph = QueryGraph.model_validate(BASE_QGRAPH)
    results = es_transpiler.convert_batch_results([qgraph], [SIMPLE_ES_HITS])
