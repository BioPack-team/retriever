...
from translator_tom import Biolink, Qualifier, QualifierConstraint, QueryGraph
...
def convert_triple_for_test(
    ...
    return es_transpiler.generate_queries(
        in_node, edge, out_node, gen_attribute_constraints=True
    )


def convert_batch_triple_for_test(
    es_transpiler: ElasticsearchTranspiler, qgraphs: list[QueryGraph]
) -> list[ESPayload]:
    return [convert_triple_for_test(es_transpiler, qgraph) for qgraph in qgraphs]


def check_list_fields(reference: list[str], against: list[str]):
...
def remove_biolink_prefixes(input: list[str]) -> list[str]:
...
def verify_expanded_multi_qualifiers(
    ...
    must_entries: list[ESEquivalentQualifierPairCollection] = generated_query["bool"][
        "must"
    ]
    ...
    for entry in must_entries:
        types_in_entry = {
            k for term in entry["bool"]["should"] for k in term["term"]
        }
...
_Side: TypeAlias = Literal["object", "subject"]


def check_single_query_payload(q_graph: QueryGraph, generated_payload: ESPayload):
    ...
    assert q_edge is not None
    sides: list[_Side] = ["subject", "object"]
    ...
    if q_edge.qualifier_constraints:
        qualifier_entries: list[QualifierConstraint] = q_edge.qualifier_constraints
        ...
        if len(qualifier_entries) > 1:
            ...
            for qualifier_entry, generated_query in zip(
                ...
                # The two helper-shape branches map 1:1 to the qualifier-set size:
                # one qualifier -> ESEquivalentQualifierPairCollection (bool.should),
                # multiple     -> ESBoolQueryForExpandedQualifiers (bool.must).
                if len(qualifiers) == 1:
                    verify_expanded_qualifier(
                        qualifiers[0],
                        cast(ESEquivalentQualifierPairCollection, generated_query),
                    )
                elif len(qualifiers) > 1:
                    verify_expanded_multi_qualifiers(
                        qualifiers,
                        cast(ESBoolQueryForExpandedQualifiers, generated_query),
                    )
        else:
            # single constraint: qualifier expansion entries are in `filter`
            qualifiers = qualifier_entries[0].qualifier_set
            bool_entries: list[ESEquivalentQualifierPairCollection] = [
                clause for clause in filter_content if "bool" in clause
            ]
    ...
    if q_edge.attribute_constraints:
    ...
    for _side in sides:
        node = q_graph.nodes[getattr(q_edge, _side)]
        if node.constraints:
            assert "must" in query_content
            assert "must_not" in query_content

            must = query_content["must"]
            assert must == [{"term": {f"{_side}.chembl_natural_product": True}}]

            must_not = query_content["must_not"]
            assert must_not == [{"term": {f"{_side}.chembl_prodrug": False}}]

    # generate check targets
    required_fields_to_check: dict[str, list[str]] = {}
    ...
    for field in EDGE_FIELDS_MAPPING:
        value = getattr(q_edge, field, None)
        if value:
            required_fields_to_check[EDGE_FIELDS_MAPPING[field]] = (
                remove_biolink_prefixes(value)
            )

    def add_to_required_fields(_field: str, _side: _Side) -> None:
        cur_node = q_graph.nodes[getattr(q_edge, _side)]
        value = getattr(cur_node, _field, None)
        if value:
            if _field != "categories" or not cur_node.ids:
                required_fields_to_check[f"{_side}.{NODE_FIELDS_MAPPING[_field]}"] = (
                    remove_biolink_prefixes(value)
                )

    for field in NODE_FIELDS_MAPPING:
        for side in sides:
...
def test_bypass_payload(es_transpiler: ElasticsearchTranspiler) -> None:
    # generated_payload = es_transpiler.convert_triple(ID_BYPASS_PAYLOAD)

    generated_payload = convert_triple_for_test(es_transpiler, ID_BYPASS_PAYLOAD)
    check_single_query_payload(ID_BYPASS_PAYLOAD, generated_payload)
    must_clauses = generated_payload["query"]["bool"]["filter"]

    # make sure id is present
    assert any(
        clause.get("terms", {}).get("subject.id") is not None for clause in must_clauses
    )

    # make sure category is NOT present
    assert not any(
        "subject.category" in clause.get("terms", {}) for clause in must_clauses
    )
...
def test_expanded_qualifier_constraints(
    ...
    """
    generated_payload = convert_triple_for_test(es_transpiler, q_graph)
    ...
    assert q_edge.qualifier_constraints is not None
    ...
    for qualifier in qualifiers:
        ...
        values = {
            next(iter(term["term"].values())) for term in expansions_by_type[qtype]
        }
...
def test_convert_triple(
    q_graph: QueryGraph, es_transpiler: ElasticsearchTranspiler
) -> None:
...
def test_convert_batch_triple(
    ...
    batch_q_graphs = [q_graph for _ in range(10)]

    # generated_payload_list = es_transpiler.convert_batch_triple(batch_q_graphs)
    generated_payload_list = convert_batch_triple_for_test(es_transpiler, batch_q_graphs)
