...
from retriever.data_tiers.tier_1.elasticsearch.qualifier_types import ESTermClause, \
...
from tests.data_tiers.tier_1.elasticsearch.payload.trapi_qgraphs import SIMPLE_QGRAPH_0
...
def check_single_query_payload(
    ...
    if 'qualifier_constraints' in q_edge:
        qualifier_entries: list[QualifierConstraintDict] = q_edge["qualifier_constraints"]
        ...
        if len(qualifier_entries) > 1:
            ...
            for qualifier_entry, generated_query in zip(qualifier_entries, should_array):
            ...
            for term in filter_content:
                ...
                if 'term' in term:
                    ...
                    for key in qualifier_query.keys():
                        assembled_value = '%'.join(
                            [
                                key, qualifier_query[key]
                            ]
                        )
