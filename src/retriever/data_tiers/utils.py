    ...
    op_hash = tomhash(
        tuple(
            sorted(
                {
                    **unhashed_op._asdict(),
                    "attributes": tuple(attr.hash() for attr in unhashed_op.attributes)
                    if unhashed_op.attributes is not None
                    else None,
                    # Sorted so the hash is independent of qualifier order.
                    "qualifiers": tuple(
                        sorted(qualifier.hash() for qualifier in unhashed_op.qualifiers)
                    )
